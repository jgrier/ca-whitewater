...
### Live flow color-coding

Each run is color-coded by current CFS:

- **white** — too low to run
- **yellow** — low but doable
- **green** — perfect
- **red** — too high
- **gray** — no live gauge (text-only readings like "Low" / "Good" from Dreamflows are shown as gray with the text)

Flow ranges per run come from Dreamflows trigger levels, AW, cacreeks, and McQuoid (for V/V+). They're encoded per *class context* — Mokelumne Electra at 1800 cfs is perfect for the Class III row and high for the Class II row.

### Refreshing the flow data

The page reads `app/flows.js`, a snapshot generated by `scripts/refresh-flows.py`. Run it to refresh:

```
python3 scripts/refresh-flows.py
```

The script pulls Dreamflows' realtime CSV (the same upstream feed used by their public site) plus the USGS instantaneous-values service for the natural-flow rivers (which is fresher, 15-min cadence). No third-party API keys, no proxies. Cron it hourly if you want continuous freshness:

```
0 * * * * cd /path/to/ca-whitewater && python3 scripts/refresh-flows.py
```

Why a refresh script and not pure live-in-browser? Many California dam-controlled rivers (Cherry Creek, MF American Oxbow, NF Feather Tobin, Lower Kern, etc.) report only via CDEC, which doesn't send CORS headers. The script reads CDEC and USGS server-side, sidestepping the browser CORS limitation.
